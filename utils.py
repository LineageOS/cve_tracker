...
def addKernel(reponame, last_update=datetime.datetime.now()):
    ...
    if v is not "error" and n is not "error":
        ...
        for c in CVE.objects():
            Patches(cve=c.id, kernel=Kernel.objects.get(repo_name=reponame).id, status=Status.objects.get(short_id=1).first().id).save()
...
def getProgress(kernel):
    patched = Patches.objects(kernel=kernel, status=Status.objects.get(short_id=2).first().id).count()
    dna = Patches.objects(kernel=kernel, status=Status.objects.get(short_id=3).first().id).count()
