	...
	function getLinks() {
		...
		$.ajax({
			...
			$.each(JSON.parse(data), function(i, v) {
				...
				var template = `<div id="${v._id.$oid}" link="${v.link}" desc="${v.desc}">
					<a class="link" href="${v.link}">${v.link}</a> - 
					<span class="linkdesc">${v.desc}</span>
					<button class="delete" link_id="${v._id.$oid}">Delete</button>
					<button class="edit" link_id="${v._id.$oid}">Edit</button>
					</div>`;
				linkList.innerHTML += template;
			});

			var deleteButtons = [].slice.call(document.querySelectorAll('#linklist .delete'));
			deleteButtons.forEach(function(button) {
				var linkId = button.getAttribute('link_id');
			...
			var editButtons = [].slice.call(document.querySelectorAll('#linklist .edit'));
