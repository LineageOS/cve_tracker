...
@app.route("/")
def index():
    min_kernel_update = datetime.datetime.now() - datetime.timedelta(weeks=26)
    kernels = Kernel.objects(last_github_update__gt=min_kernel_update, deprecated__in=[False, None]).order_by('vendor', 'device')
